...
<el-row>
  ...
  <el-col :span="8" style='margin-top:15px;'>
    <el-input prefix-icon="wk-icon icon-search" placeholder="输入关键字进行过滤" v-model="filterText"></el-input>
    <el-tree class="filter-tree"  :data="treeData" node-key="id" highlight-current :props="defaultProps"  :filter-node-method="filterNode" ref="groupTree" @node-click="getNodeData" default-expand-all> 
      <span class="custom-tree-node" slot-scope="{ node ,data}">
            <span>
                <i :class="data.children.length==0?'el-icon-tickets':'el-icon-folder'"></i>{{ node.label }}
            </span>              
        </span>
    </el-tree>
  </el-col>
  ...
  <el-dialog v-dialogDrag :title="dialogUserName" :visible.sync="dialogUserVisible">
    <group-user :groupId="currentId" @closeUserDialog="closeUserDialog" ref="groupUser"></group-user>
  </el-dialog>
  <el-dialog v-dialogDrag :title="dialogAuthorityName" size="large" :visible.sync="dialogAuthorityVisible">
